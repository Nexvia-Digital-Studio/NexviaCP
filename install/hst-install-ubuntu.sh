...
if [ "$docker" = 'yes' ]; then
	...
	if [ -n "$os_codename" ] && [ -f /etc/apt/keyrings/docker.asc ]; then
		echo "deb [arch=$(dpkg --print-architecture 2>/dev/null) signed-by=/etc/apt/keyrings/docker.asc] https://download.docker.com/linux/ubuntu $os_codename stable" \
...
if [ "$dotnet" = 'yes' ]; then
	...
	if [ -n "$os_codename" ] && [ -f /usr/share/keyrings/microsoft.asc ]; then
		echo "deb [arch=$(dpkg --print-architecture 2>/dev/null) signed-by=/usr/share/keyrings/microsoft.asc] https://packages.microsoft.com/ubuntu/${os_codename}/prod $os_codename main" \
