...
<div class="container">
	...
	>
		...
		<div class="form-container">
			...
			<div class="card u-mb20" style="border-left: 4px solid #3b82f6; background: #ffffff; border-radius: 8px; border: 1px solid #d3d3d3; box-shadow: 0 2px 6px rgba(0,0,0,0.04);">
				<div style="display: flex; justify-content: space-between; align-items: center; gap: 12px; padding: 14px 18px; border-bottom: 1px solid #e2e8f0; flex-wrap: wrap; background: #f8fafc;">
					<div style="display: flex; align-items: center; gap: 10px; flex-wrap: wrap;">
						<i class="fas fa-hourglass-half fa-lg" style="color: #3b82f6;"></i>
						<span style="font-weight: 700; font-size: 14px; color: #1e293b;"><?= tohtml(function_exists('__tr') ? __tr("Domain Validity & Subscription Lifecycle", "Domain Geçerlilik & Lisans Süresi Yönetimi") : "Domain Expiry & Subscription") ?></span>
					...
					<div>
						<?php if ($v_expiry_status === 'unlimited') { ?>
							<span style="background: #ede9fe; color: #6d28d9; padding: 5px 12px; border-radius: 99px; font-weight: 700; font-size: 12px;"><i class="fas fa-infinity u-mr5"></i><?= tohtml(function_exists('__tr') ? __tr("Unlimited (Lifetime)", "Sınırsız / Süresiz Lisans") : "Unlimited") ?></span>
						<?php } elseif ($v_expiry_status === 'expired') { ?>
							<span style="background: #fee2e2; color: #991b1b; padding: 5px 12px; border-radius: 99px; font-weight: 700; font-size: 12px;"><i class="fas fa-circle-xmark u-mr5"></i><?= tohtml(function_exists('__tr') ? __tr("Expired (Redirect & Site Halted)", "Süresi Doldu (Yayın ve Yönlendirme Kapalı)") : "Expired") ?></span>
						<?php } elseif ($v_expiry_status === 'expiring_soon') { ?>
							<span style="background: #fef3c7; color: #92400e; padding: 5px 12px; border-radius: 99px; font-weight: 700; font-size: 12px;"><i class="fas fa-clock u-mr5"></i><?= tohtml($v_days_left) ?> <?= tohtml(function_exists('__tr') ? __tr("days left (Expiring Soon!)", "gün kaldı (Yakında Bitiyor!)") : "days left") ?></span>
						<?php } else { ?>
							<span style="background: #dcfce7; color: #166534; padding: 5px 12px; border-radius: 99px; font-weight: 700; font-size: 12px;"><i class="fas fa-circle-check u-mr5"></i><?= tohtml($v_days_left) ?> <?= tohtml(function_exists('__tr') ? __tr("days left", "gün kaldı") : "days left") ?></span>
						<?php } ?>
					</div>
				</div>
				<div style="padding: 16px 18px;">
					<div style="display: flex; gap: 20px; flex-wrap: wrap; margin-bottom: 16px; background: #f8fafc; padding: 12px 16px; border-radius: 6px; border: 1px solid #e2e8f0;">
						<div>
							<small style="color: #64748b; display: block; font-weight: 600; margin-bottom: 3px;"><?= tohtml(function_exists('__tr') ? __tr("Creation Date", "Oluşturulma / Ekleme Tarihi") : "Creation Date") ?></small>
							<span style="font-weight: 700; color: #334155; font-size: 13px;"><?= tohtml($v_created_date ?: "-") ?> <?= tohtml($v_created_time ?: "") ?></span>
						</div>
						<div>
							<small style="color: #64748b; display: block; font-weight: 600; margin-bottom: 3px;"><?= tohtml(function_exists('__tr') ? __tr("Expiration Date", "Bitiş / Lisans Tarihi") : "Expiration Date") ?></small>
							<span style="font-weight: 700; color: <?= ($v_expiry_status === 'expired') ? '#dc2626' : '#1e293b' ?>; font-size: 13px;">
								<?= tohtml($v_expiry_date === 'unlimited' ? (function_exists('__tr') ? __tr("Unlimited", "Süresiz") : "Unlimited") : $v_expiry_date) ?>
							</span>
						</div>
						<div>
							<small style="color: #64748b; display: block; font-weight: 600; margin-bottom: 3px;"><?= tohtml(function_exists('__tr') ? __tr("Website & Routing Status", "Yayın ve Yönlendirme Durumu") : "Routing Status") ?></small>
							<?php if ($v_suspended === 'yes') { ?>
								<span style="color: #dc2626; font-weight: 700; font-size: 13px;"><i class="fas fa-ban u-mr5"></i><?= tohtml(function_exists('__tr') ? __tr("Suspended (Stopped)", "Askıda / Durduruldu") : "Suspended") ?></span>
							<?php } else { ?>
								<span style="color: #16a34a; font-weight: 700; font-size: 13px;"><i class="fas fa-circle-check u-mr5"></i><?= tohtml(function_exists('__tr') ? __tr("Active & Live", "Aktif ve Yayında") : "Active") ?></span>
							<?php } ?>
						</div>
					</div>
					...
					<form method="post" action="/edit/web/?domain=<?= tohtml(urlencode(trim($v_domain, "'"))) ?>&token=<?= tohtml($_SESSION['token']) ?>" style="margin: 0;">
						...
						<label class="form-label" style="font-size: 13px; font-weight: 700; margin-bottom: 8px; display: block;">
							<i class="fas fa-plus-circle u-mr5" style="color: #2563eb;"></i><?= tohtml(function_exists('__tr') ? __tr("Quick Duration Extension / Renewal (Hızlı Süre Tanımla / Uzat)", "Hızlı Süre Tanımla / Uzat") : "Extend Duration") ?>
						</label>
						<div style="display: flex; gap: 8px; flex-wrap: wrap; align-items: center;">
							<button type="submit" name="quick_expiry_value" value="3d" class="button button-secondary" style="font-size: 12px; padding: 6px 12px;" title="3 Günlük Demo / Deneme">
								⏳ +3 Gün (Demo)
							</button>
							<button type="submit" name="quick_expiry_value" value="1m" class="button button-secondary" style="font-size: 12px; padding: 6px 12px;" title="1 Aylık Satış / Uzatma">
								🗓️ +1 Ay
							</button>
							<button type="submit" name="quick_expiry_value" value="1y" class="button" style="font-size: 12px; padding: 6px 14px; background: #2563eb; color: #fff; font-weight: 700;" title="1 Yıllık Standart Satış">
								📅 +1 Yıl (Standart Satış)
							</button>
							<button type="submit" name="quick_expiry_value" value="5y" class="button button-secondary" style="font-size: 12px; padding: 6px 12px;" title="5 Yıllık Lisans">
								📅 +5 Yıl
							</button>
							<button type="submit" name="quick_expiry_value" value="unlimited" class="button button-secondary" style="font-size: 12px; padding: 6px 12px;" title="Süresiz / Sınırsız Yap">
								♾️ Sınırsız Yap
							</button>
							<button type="button" class="button button-secondary" style="font-size: 12px; padding: 6px 12px;" onclick="document.getElementById('edit-custom-exp-box').style.display = (document.getElementById('edit-custom-exp-box').style.display === 'none' ? 'flex' : 'none');">
								✏️ Özel Tarih…
							</button>
						</div>
						<div id="edit-custom-exp-box" style="display: none; gap: 8px; align-items: center; margin-top: 12px; background: #f1f5f9; padding: 10px 14px; border-radius: 6px;">
							...
							<button type="submit" name="quick_expiry_value" value="custom" class="button" style="font-size: 12px; padding: 6px 14px;">
